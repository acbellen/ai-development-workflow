...
Use one bounded real call first, on the exact id you are about to dispatch: availability and limits are per model id and account tier, so a cheaper id refused with an account-tier error reads as a dead vendor and a working one proves nothing about the route you will actually buy. Diagnose a failed call before choosing a fallback; a timeout does not prove exhausted credit. Preserve rejected output and any valid unresolved findings. Never automatically repeat implementation after partial edits: inspect and resume the actual worktree. If no independent route is available, leave the review pending/draft and report the limitation.
